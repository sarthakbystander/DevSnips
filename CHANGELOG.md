...
### Added — React Sidebar component family (12 variants)
- Added `React/Components/Sidebar/` — a compound, TypeScript-first, Tailwind-first application-navigation family: a persistent desktop sidebar (expanded 256px ↔ collapsed 64px icon rail) plus a mobile modal drawer rendered from the SAME composition of children. All 12 variants ship the standard `{code.tsx, code.jsx, preview.html, metadata.json, README.md}` asset shape with a single shared core (`sidebar/code.tsx` is the authored reference; the other 11 are derived by `_gen_react_sidebar.py` from `_gen_react_sidebar_registry.py`, header-neutralized equality enforced).
- Variants: `sidebar` (reference), `sidebar-collapsed` (controlled icon rail: measured fixed-position tooltips, sr-only labels, badge dots, one-step expand-and-open for parents), `sidebar-mobile` (controlled drawer with an event log), `sidebar-with-groups` (labeled sections), `sidebar-with-nested-navigation` (disclosure parents, 3-level cap, disabled nested row), `sidebar-with-active-state` (data-driven route-active model, auto-expanding parents), `sidebar-with-badges` (live count updates, status chips), `sidebar-with-user` (rail-adaptive identity block + account actions), `sidebar-with-footer-actions` (real links vs real buttons, working sign-out feedback), `sidebar-with-search` (real filtering, parent-preserving matches, status empty state, clear/Escape reset), `sidebar-collapsible-groups` (independent controlled groups + external expand/collapse-all), `sidebar-dashboard` (flagship full console composition).
- 19 exports: `SidebarProvider` (state root: controlled/uncontrolled `collapsed` + `mobileOpen`, breakpoint, single matchMedia listener, drawer Escape/scroll-lock/focus-restoration), `Sidebar` (desktop `<aside>` landmark + `role="dialog" aria-modal` drawer from one composition), `SidebarHeader`/`SidebarContent`/`SidebarFooter`, `SidebarGroup`/`SidebarGroupLabel`, `SidebarMenu`/`SidebarMenuItem`/`SidebarMenuButton`, `SidebarMenuCollapsible`, `SidebarMenuSub`/`SidebarMenuSubItem`/`SidebarMenuSubButton`, `SidebarTrigger`, `SidebarRail`, `SidebarSearch`, `SidebarNav` (typed `SidebarNavItem`/`SidebarNavSection` data renderer with query filtering), `useSidebar`.
- Behavior: `aria-current="page"` + surface + inset indicator bar for the current page; weight-only parent indication; drawer focus trap (Tab wrap), focus-into-drawer on open, focus restore to the trigger, overlay/Escape/close-button/link close paths, body scroll lock with scrollbar compensation, auto-close on resize to desktop; collapsed rail keeps every label in the accessibility tree (sr-only) and exposes measured tooltips on hover/focus (DOM style API, no inline `style=`); badges collapse to dots with sr-only counts.
- QA: `scripts/_qa_react_sidebar.py` (static shape/metadata/export-parity/shared-core/no-any/no-hex/anti-AI + generator `--check` + `scripts/validate.py` subprocess gates, full Playwright behavior per variant, drawer dialog semantics, tooltips, search, controlled coordination, dark mode, reduced motion, zero overflow @ 375/768/1280) — 1811 checks passed. TSX strict-checked with `tsc` (typescript@5.9 in /tmp/dsbuild, noUncheckedIndexedAccess, build-time-only). `python3 _gen_react_sidebar.py --check` drift-free. React is still not a registered technology in `snippets-index.json` (follow-up; `scripts/validate.py` passes).
