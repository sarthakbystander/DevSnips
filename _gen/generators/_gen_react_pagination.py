#!/usr/bin/env python3
"""DevSnips React Pagination generator.

For every pagination variant in ``React/Components/Pagination/`` this
generator:
...
Pagination is a multi-export compound family (`Pagination`,
`PaginationContent`, `PaginationItem`, `PaginationLink`, `PaginationPrevious`,
`PaginationNext`, `PaginationEllipsis`, plus `PaginationStatus` on the
previous/next variant, `getPaginationRange` + `PaginationPages` on the
ellipsis variant, and `PaginationPageSize` on the page-size variant), so it
reuses the tabs/breadcrumbs multi-export esbuild parity conversion.

Author ``code.tsx`` per variant, register metadata in
``_gen/registries/_gen_react_pagination_registry.py``, then run:

    python3 _gen/generators/_gen_react_pagination.py            # write everything
    python3 _gen/generators/_gen_react_pagination.py --check    # report drift, no writes
...
# Base Icon set + the breadcrumb navigation glyphs, plus a "file" glyph the
# pagination showcases use (files, documents, activity entries).
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Pagination / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Pagination · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
COMPOSITION = """Pagination is a compound component. Seven primitives compose the pattern:
...
LOGIC_BASE = """The root `<Pagination>` owns the current page and clamps every navigation target into `1 … totalPages`, so out-of-range requests are impossible. Both modes are supported:
...
STATES = """- **Page control (idle)** — muted foreground; hover shifts to a subtle surface with foreground text.
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_pagination_registry.py"
        spec = importlib.util.spec_from_file_location("_gen_react_pagination_registry", reg)
        mod = importlib.util.module_from_spec(spec)
        _sys = sys
        _sys.modules["_gen_react_pagination"] = _sys.modules[__name__]
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} pagination variants up to date.")
    else:
        print(f"Wrote {len(COMPONENTS)} pagination variants.")
