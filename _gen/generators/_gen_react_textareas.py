#!/usr/bin/env python3
"""DevSnips React Textareas generator.

For every textarea in ``React/Components/Textareas/`` this generator:
...
``_gen/registries/_gen_react_textareas_registry.py``, then run:

    python3 _gen/generators/_gen_react_textareas.py            # write everything
    python3 _gen/generators/_gen_react_textareas.py --check    # report drift, no writes
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Textareas / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Textareas · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_textareas_registry.py"
        spec = importlib.util.spec_from_file_location("_gen_react_textareas_registry", reg)
        mod = importlib.util.module_from_spec(spec)
        import sys as _s
        _s.modules["_gen_react_textareas"] = _s.modules[__name__]
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} textareas up to date.")
    else:
        print(f"Wrote {len(COMPONENTS)} textareas.")
