#!/usr/bin/env python3
"""DevSnips React Checkboxes generator.

For every checkbox in ``React/Components/Checkboxes/`` this generator:
...
``_gen/registries/_gen_react_checkboxes_radios_registry.py``, then run:

    python3 _gen/generators/_gen_react_checkboxes.py            # write everything
    python3 _gen/generators/_gen_react_checkboxes.py --check    # report drift, no writes
...
CHECKBOXES = ROOT / "React/Components/Checkboxes"
...
def render_preview(folder: Path, slug: str, spec: dict) -> str:
    ...
    for dep_slug in spec.get("extra", []):
        if dep_slug in COMPONENTS and dep_slug != slug:
            dep_folder = CHECKBOXES / dep_slug
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Checkboxes / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Checkboxes · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_checkboxes_radios_registry.py"
        spec = importlib.util.spec_from_file_location("_gen_react_checkboxes_radios_registry", reg)
        mod = importlib.util.module_from_spec(spec)
        import sys as _s
        _s.modules["_gen_react_checkboxes"] = _s.modules[__name__]
        spec.loader.exec_module(mod)
    drift = []
    for slug, spec in COMPONENTS.items():
        folder = CHECKBOXES / slug
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} checkboxes up to date.")
    else:
        print(f"Wrote {len(COMPONENTS)} checkboxes.")
