#!/usr/bin/env python3
"""DevSnips React Selects generator.

For every select in ``React/Components/Selects/`` this generator:
...
``_gen/registries/_gen_react_selects_registry.py``, then run:

    python3 _gen/generators/_gen_react_selects.py            # write everything
    python3 _gen/generators/_gen_react_selects.py --check    # report drift, no writes
...
SELECTS = ROOT / "React/Components/Selects"
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Selects / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Selects · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
def render_metadata(slug: str, spec: dict) -> str:
    return json.dumps({
        "id": f"{slug}-react-001",
        "name": spec["title"],
        "slug": slug,
...
def render_readme(slug: str, spec: dict) -> str:
    name = spec["title"]
    usage = spec["props_doc"].get("usage", f"<{name} />")
    table = spec["props_doc"].get("table", "")
    return f"""# {name}
...
```tsx
import {{ {spec['props_doc'].get('export_name', name.replace(' ', ''))} }} from './code';
...
def main(check: bool = False) -> None:
    # Load the registry so register() calls populate COMPONENTS.
    import importlib.util
    reg = ROOT / "_gen" / "registries" / "_gen_react_selects_registry.py"
    spec = importlib.util.spec_from_file_location("_gen_react_selects_registry", reg)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["_gen_react_selects"] = sys.modules[__name__]
    spec.loader.exec_module(mod)

    drift = []
    wrote = 0
    for slug, cspec in COMPONENTS.items():
        folder = SELECTS / slug
        if not (folder / "code.tsx").exists():
            print(f"ERROR: missing code.tsx for {slug}")
            drift.append(str(folder / "code.tsx"))
            continue
        files = {
            "code.jsx": render_code_jsx(folder),
            "preview.html": render_preview(folder, slug, cspec),
            "metadata.json": render_metadata(slug, cspec),
            "README.md": render_readme(slug, cspec),
        }
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} selects up to date.")
    else:
        print(f"Wrote {wrote} selects (code.jsx, preview.html, metadata.json, README.md).")
