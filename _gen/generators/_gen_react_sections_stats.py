#!/usr/bin/env python3
"""Generate preview.html for the React Sections Stats family.

Reads the authored `React/Sections/Stats/<slug>/code.tsx` (the single
...
Hero/Features/Testimonials/Pricing sections generators
(`_gen_react_sections_hero.py`, `_gen_react_sections_features.py`,
`_gen_react_sections_testimonials.py`, `_gen_react_sections_pricing.py`):
  ...
  - the actual component transformed from code.tsx via esbuild
    (`_tsx_to_babel_component`), exposed on `window.StatsSection`
...
Usage:
    python3 _gen/generators/_gen_react_sections_stats.py          # write all 4 previews
    python3 _gen/generators/_gen_react_sections_stats.py --check  # detect drift, exit 1
"""
...
STATS_DIR = ROOT / "React" / "Sections" / "Stats"
...
# design direction, like the other section families.
VARIANTS = {
    "minimal": (
        "Stats — Minimal",
        "The reference composition: a left-aligned header block above a "
        "ruled stat rail — four hairline-separated metrics with strong "
        "numerical hierarchy, mono labels, and no cards. Compact rhythm, "
        "static by design.",
    ),
    "dark-premium": (
        "Stats — Dark Premium",
        "An asymmetric 4/8 split on a pinned dark canvas: header and one "
        "quiet link beside a raised telemetry panel — two featured "
        "metrics above a ruled definition list. The section pins the dark "
        "theme mapping on its own root — toggle the page theme to see "
        "it hold.",
    ),
    "bento": (
        "Stats — Bento",
        "Statistics as a modular 12-column cell grid: one hero anchor "
        "cell, one accent highlight cell, and two supporting cells. "
        "Uniform gap, one radius, border-only hover lift, intentionally "
        "action-free.",
    ),
    "neo-brutalist": (
        "Stats — Neo-Brutalist",
        "The expressive ceiling, kept disciplined: four square stat "
        "blocks with uniform 2px borders, hard offset shadows, mono "
        "uppercase labels, one flat accent fill, and a press-down action.",
    ),
}
...
def render_preview(slug: str, tsx: str) -> str:
    ...
    component_js = _tsx_to_babel_component(tsx, expose_name="StatsSection")
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Sections / Stats / {slug}</span></div>
    <button class="ds-theme-toggle" id="ds-theme-toggle" type="button" aria-pressed="false">
      <span id="ds-theme-label">Dark</span>
    </button>
  </header>
  <div class="ds-intro">
    <p class="ds-eyebrow">React Sections · Stats</p>
  ...
  <footer class="ds-footer">DevSnips React · Sections · Stats · <code>{slug}</code> · live render of code.tsx</footer>
</div>
<script>
(function(){{
...
ReactDOM.createRoot(document.getElementById("ds-root")).render(<StatsSection />);
</script>
</body>
</html>
"""
...
def main() -> int:
    ...
    if drift:
        ...
        print("Run: python3 _gen/generators/_gen_react_sections_stats.py")
        return 1
    if args.check:
        print("All Stats previews are up to date.")
    return 0
