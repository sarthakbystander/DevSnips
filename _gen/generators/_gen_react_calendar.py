...
``tsx_header`` in ``_gen/registries/_gen_react_calendar_registry.py``, then run:

    python3 _gen/generators/_gen_react_calendar.py            # write everything
    python3 _gen/generators/_gen_react_calendar.py --check    # report drift, no writes
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_calendar_registry.py"
