#!/usr/bin/env python3
"""Generate preview.html for the React Sections Logo-Cloud family.

Reads the authored `React/Sections/Logo-Cloud/<slug>/code.tsx` (the
...
Hero/Features/Testimonials/Pricing/Stats sections generators:
  ...
  - the actual component transformed from code.tsx via esbuild
    (`_tsx_to_babel_component`), exposed on `window.LogoCloudSection`
...
Usage:
    python3 _gen/generators/_gen_react_sections_logo_cloud.py          # write all 4 previews
    python3 _gen/generators/_gen_react_sections_logo_cloud.py --check  # detect drift, exit 1
"""
...
LOGO_CLOUD_DIR = ROOT / "React" / "Sections" / "Logo-Cloud"
...
VARIANTS = {
    "minimal": (
        "Logo Cloud — Minimal",
        "The reference composition: a compact, left-aligned header block "
        "above a full-width hairline brand wall — invented wordmarks in "
        "uniform cells separated by 1px rules alone, never cards. Clean, "
        "monochrome, typography-driven.",
    ),
    "dark-premium": (
        "Logo Cloud — Dark Premium",
        "A brand-wall panel on a pinned dark canvas: an asymmetric 4/8 "
        "split with the header and one quiet link beside a single raised "
        "panel holding a uniform ruled wall of wordmarks in the muted "
        "ramp. The section pins the dark theme mapping on its own root — "
        "toggle the page theme to see it hold.",
    ),
    "bento": (
        "Logo Cloud — Bento",
        "An asymmetric brand ecosystem in a 12-column cell grid: one hero "
        "brand cell with a larger visual presence, one integration-count "
        "statement cell carrying the single accent, and seven supporting "
        "brand cells. Uniform gap, one radius, border-only hover lift, "
        "intentionally action-free.",
    ),
    "neo-brutalist": (
        "Logo Cloud — Neo-Brutalist",
        "The expressive ceiling, kept disciplined: six square brand cells "
        "with uniform 2px borders, hard offset shadows, mono index "
        "numbers, uppercase mono wordmarks, one flat accent fill, and a "
        "press-down action.",
    ),
}
...
def render_preview(slug: str, tsx: str) -> str:
    ...
    component_js = _tsx_to_babel_component(tsx, expose_name="LogoCloudSection")
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Sections / Logo-Cloud / {slug}</span></div>
    <button class="ds-theme-toggle" id="ds-theme-toggle" type="button" aria-pressed="false">
      <span id="ds-theme-label">Dark</span>
    </button>
  </header>
  <div class="ds-intro">
    <p class="ds-eyebrow">React Sections · Logo Cloud</p>
  ...
  <footer class="ds-footer">DevSnips React · Sections · Logo Cloud · <code>{slug}</code> · live render of code.tsx</footer>
</div>
<script>
(function(){{
...
ReactDOM.createRoot(document.getElementById("ds-root")).render(<LogoCloudSection />);
</script>
</body>
</html>
"""
...
def main() -> int:
    ...
    if drift:
        ...
        print("Run: python3 _gen/generators/_gen_react_sections_logo_cloud.py")
        return 1
    if args.check:
        print("All Logo Cloud previews are up to date.")
    return 0
