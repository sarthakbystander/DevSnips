#!/usr/bin/env python3
"""Generate preview.html for the React Sections FAQ family.

Reads the authored `React/Sections/FAQ/<slug>/code.tsx` (the single source of
...
React Components (`_gen_react_buttons.py`) and the Hero/Features/Testimonials
Pricing/Stats/Logo-Cloud sections generators:
  ...
  -the actual component transformed from code.tsx via esbuild
    (`_tsx_to_babel_component`), exposed on `window.FAQSection`
...
Usage:
    python3 _gen/generators/_gen_react_sections_faq.py          # write all 4 previews
    python3 _gen/generators/_gen_react_sections_faq.py --check  # detect drift, exit 1
"""
...
FAQ_DIR = ROOT / "React" / "Sections" / "FAQ"
...
VARIANTS = {
    "minimal": (
        "FAQ — Minimal",
        "The reference composition: a restrained, editorial question list."
        "A left-aligned header block above a narrow single column of questions"
        "— each trigger a thin hairline between, never a card. Typography,"
        "spacing,and one quiet contact link carry the design. The accordion"
        "is keyboard-operable and reduced-motion safe.",
    ),
    "dark-premium": (
        "FAQ — Dark Premium",
        "An asymmetric 4/8 editorial split on a pinned dark canvas: the"
        "oversized heading block and a ruled \u201cStill have questions?\u201d"
        "aside sit left; a single raised panel with the answered question list"
        "sits right, one elevation step above the canvas. The section pins"
        "the dark theme mapping on its own root — toggle the page theme to"
        "see it hold.",
    ),
    "bento": (
        "FAQ — Bento",
        "A genuine 12-column bento composition: large heading cell"
        "(span 5, the FAQ accordion cell (span 7) as the primary focus,"
        "a contact cell (span 5),and a secondary \u201cGood to know\u201d cell"
        "(span 7) of quick reference facts. Uniform gap, one radius,"
        "border-only hover lift, one quiet contact action.",
    ),
    "neo-brutalist": (
        "FAQ — Neo-Brutalist",
        "The expressive ceiling, kept disciplined: FAQ items as square"
        "2px-bordered interface blocks with hard offset shadows, mono index"
        "badges, bold questions,and a press-down trigger that translates by"
        "its shadow offset. One filled primary action;everything else"
        "stays flat surface.",
    ),
}
...
def render_preview(slug: str, tsx: str) -> str:
    ...
    component_js = _tsx_to_babel_component(tsx, expose_name="FAQSection")
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Sections / FAQ / {slug}</span></div>
    <button class="ds-theme-toggle" id="ds-theme-toggle" type="button" aria-pressed="false">
      <span id="ds-theme-label">Dark</span>
    </button>
  </header>
  <div class="ds-intro">
    <p class="ds-eyebrow">React Sections · FAQ</p>
  ...
  <footer class="ds-footer">DevSnips React · Sections · FAQ · <code>{slug}</code> · live render of code.tsx</footer>
</div>
<script>
(function(){{
...
ReactDOM.createRoot(document.getElementById("ds-root")).render(<FAQSection />);
</script>
</body>
</html>
"""
...
def main() -> int:
    ...
    if drift:
        ...
        print("Run: python3 _gen/generators/_gen_react_sections_faq.py")
        return 1
    if args.check:
        print("All FAQ previews are up to date.")
    return 0
