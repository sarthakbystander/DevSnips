...
``tsx_header`` in ``_gen/registries/_gen_react_dialogs_registry.py``, then run:

    python3 _gen/generators/_gen_react_dialogs.py            # write everything
    python3 _gen/generators/_gen_react_dialogs.py --check    # report drift, no writes
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_dialogs_registry.py"
