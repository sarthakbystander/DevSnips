#!/usr/bin/env python3
"""DevSnips React Switches generator.

For every switch in ``React/Components/Switches/`` this generator:
...
``_gen/registries/_gen_react_switches_registry.py``, then run:

    python3 _gen/generators/_gen_react_switches.py            # write everything
    python3 _gen/generators/_gen_react_switches.py --check    # report drift, no writes
...
SWITCHES = ROOT / "React/Components/Switches"
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Switches / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Switches · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_switches_registry.py"
        spec = importlib.util.spec_from_file_location("_gen_react_switches_registry", reg)
        mod = importlib.util.module_from_spec(spec)
        import sys as _s
        _s.modules["_gen_react_switches"] = _s.modules[__name__]
        spec.loader.exec_module(mod)
    drift = []
    for slug, spec in COMPONENTS.items():
        folder = SWITCHES / slug
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} switches up to date.")
    else:
        print(f"Wrote {len(COMPONENTS)} switches.")
