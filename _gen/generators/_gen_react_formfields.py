#!/usr/bin/env python3
"""DevSnips React Form Fields generator.

For every form-field variant in ``React/Components/FormFields/`` this
generator:
  - reads the authored reference ``form-field/code.tsx`` (the primary,
...
Form Fields is a multi-export compound family (`FormField`,
`FormFieldLabel`, `FormFieldControl`, `FormFieldDescription`,
`FormFieldHelper`, `FormFieldMessage`, `FormFieldGroup`, `useFormField`), so
it reuses the tabs/breadcrumbs/pagination/dropdowns/dialogs multi-export
esbuild parity conversion. No portals are involved, so no `react-dom`
import mapping is needed.

Author the reference ``form-field/code.tsx``, register metadata + showcase +
``tsx_header`` in ``_gen/registries/_gen_react_formfields_registry.py``, then run:

    python3 _gen/generators/_gen_react_formfields.py            # write everything
    python3 _gen/generators/_gen_react_formfields.py --check    # report drift, no writes
...
# Base Icon set + the form-field glyphs the showcases use (mail, lock,
# globe, info).
_EXTRA_ICON_CASES = (
    '    case "mail": return (<svg {...common}><rect width="20" height="16" x="2" y="4" rx="2"/>'
    '<path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>);\n'
    '    case "lock": return (<svg {...common}><rect width="18" height="11" x="3" y="11" rx="2"/>'
    '<path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>);\n'
    '    case "globe": return (<svg {...common}><circle cx="12" cy="12" r="10"/>'
...
def register(slug, *, title, subcategory, description, tags, features,
             ...
             composition_note, wiring_doc, a11y_doc, states_doc,
             responsive_doc, notes_doc, tsx_header, showcase):
    COMPONENTS[slug] = dict(
        ...
        wiring_doc=wiring_doc, a11y_doc=a11y_doc, states_doc=states_doc,
...
def read_reference_tsx() -> str:
    return (FORMFIELDS / REFERENCE / "code.tsx").read_text(encoding="utf-8").strip("\n") + "\n"
...
<div class="ds-page">
  <header class="ds-topbar">
    <div class="ds-brand"><span class="ds-mark" aria-hidden="true">D</span><span>DevSnips</span><span class="ds-crumb" aria-hidden="true">/ <b>React</b> / Form Fields / {slug}</span></div>
  ...
  <footer class="ds-footer">DevSnips React · Form Fields · <code>{slug}</code> · preview demonstration of code.tsx</footer>
</div>
<script>
(function(){{
...
COMPOSITION = """- `FormField` — the root provider. Owns the control id (generated or via `controlId`), the `required` / `disabled` / `orientation` state, and the registry that description, helper, and message texts register into.
...
def main(check=False):
    if not COMPONENTS:
        ...
        reg = ROOT / "_gen" / "registries" / "_gen_react_formfields_registry.py"
        spec = importlib.util.spec_from_file_location("_gen_react_formfields_registry", reg)
        mod = importlib.util.module_from_spec(spec)
        _sys = sys
        _sys.modules["_gen_react_formfields"] = _sys.modules[__name__]
    ...
    if check:
        if drift:
            ...
            for d in drift:
                print("  " + d)
            sys.exit(1)
        print(f"OK: {len(COMPONENTS)} form field variants up to date.")
    else:
        print(f"Wrote {len(COMPONENTS)} form field variants.")
