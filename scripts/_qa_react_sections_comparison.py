...
def static_checks() -> None:
    ...
    check(sorted(p.name for p in root.iterdir() if p.is_dir()) == SLUGS, "exact four direction folders")
    for slug in SLUGS:
        folder = root / slug
        ...
        check(not hex_lit.search(tsx.replace("#000", "")), f"{slug}: no raw hex outside sanctioned #000")
        ...
        check(meta.get("subcategory") == "Comparison", f"{slug}: metadata subcategory")
        check(meta.get("type") == "section", f"{slug}: metadata type")
        check(meta.get("category") == "Sections", f"{slug}: metadata category")
        check(meta.get("direction") in ["Minimal", "Dark Premium", "Bento", "Neo-Brutalist"], f"{slug}: metadata direction")
    ...
    with sync_playwright() as pw:
        ...
        for slug in SLUGS:
            ...
            if slug == "dark-premium":
                check(section.locator("dl").count() == 3, f"{slug}: stacked comparison panels use semantic dl")
                check(section.get_attribute("data-theme") == "dark", f"{slug}: section pins dark theme")
            if slug == "bento":
                check(section.locator(".md\\:grid-cols-12").count() == 1, f"{slug}: 12-column bento grid present")
            for width in WIDTHS:
                page.set_viewport_size({"width": width, "height": 900})
                for theme in ["light", "dark"]:
                    page.evaluate("t=>document.documentElement.setAttribute('data-theme',t)", theme)
                    overflow = page.evaluate("document.documentElement.scrollWidth-document.documentElement.clientWidth")
                    check(overflow <= 0, f"{slug}: no page overflow @ {width}px {theme} (got {overflow})")
            page.set_viewport_size({"width": 1280, "height": 900})
            def colors(theme: str):
                page.evaluate("t=>document.documentElement.setAttribute('data-theme',t)", theme)
                return section.evaluate("e=>{let c=getComputedStyle(e);return [c.backgroundColor,c.color]}")
            light = colors("light")
            dark = colors("dark")
            if slug == "dark-premium":
                check(light == dark, f"{slug}: pinned dark colors persist across page themes")
                check(light[0] not in ["rgb(250, 250, 250)", "rgb(255, 255, 255)"], f"{slug}: actually dark")
            else:
            ...
            check(section.locator(":focus").count() > 0, f"{slug}: keyboard navigation reaches section controls")
            page.emulate_media(reduced_motion="reduce")
            check(first.evaluate("e=>getComputedStyle(e).transitionDuration") in ["0s", "0.01s"], f"{slug}: reduced motion removes transition")
