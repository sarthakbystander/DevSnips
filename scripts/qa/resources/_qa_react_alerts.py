...
def static_checks():
    ...
    for slug in VARIANTS:
        folder = ROOT / "library/React/Components/Alerts" / slug
    ...
    css = list((ROOT / "library/React/Components/Alerts").rglob("*.css"))
    ...
    reference = (ROOT / "library/React/Components/Alerts/alert/code.tsx").read_text()
    ref_body = re.sub(r"/\*\*.*?\*/", "", reference, count=1, flags=re.S)
    for slug in VARIANTS[1:]:
        tsx = (ROOT / "library/React/Components/Alerts" / slug / "code.tsx").read_text()
