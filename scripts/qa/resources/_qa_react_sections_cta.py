#!/usr/bin/env python3
"""Playwright QA for the React Sections CTA previews.
...
Verifies behavior-critical guarantees (not cosmetics):
  ...
  - structure: exactly one h2 per section, section aria-labelledby points at
    it,,and the expected CTA action labels render (primary + secondary). All
    four variants ship real action links, so every link gets a focus-
    visible ring check.
  ...
  - generator: `scripts/tooling/generators/_gen_react_sections_cta.py --check` reports no drift;
...
Run from the repo root with a static server on :8765:
    ...
    python3 scripts/_qa_react_sections_cta.py
"""
import json
import re
import subprocess
...
BASE = "http://localhost:8765/React/Sections/CTA/"
SLUGS = [
    "minimal",
    "dark-premium",
    "bento",
...
EXPECTED_LABELS = {
    "minimal": ["Explore components", "Read the docs"],
    "dark-premium": ["Start a private beta", "Talk to engineering"],
    "bento": ["Explore components", "Read the docs"],
    "neo-brutalist": ["Start shipping", "Read the runbook"],
}
...
def static_checks() -> None:
    ...
    family_dir = ROOT / "library" / "React" / "Sections" / "CTA"
    ...
    for slug in SLUGS:
        ...
        folder = ROOT / "library" / "React" / "Sections" / "CTA" / slug
        ...
        check(meta.get("family") == "CTA", f"{slug}: metadata family")
        check(meta.get("subcategory") == "CTA", f"{slug}: metadata subcategory")
        ...
        check(meta.get("id") == f"cta-{slug}", f"{slug}: metadata id")
        for label in EXPECTED_LABELS[slug]:
            check(
                label in tsx,
                f"{slug}: authored action label present `{label}`",
            )
...
def browser_checks() -> None:
    with sync_playwright() as pw:
        ...
        for slug in SLUGS:
            ...
            for label in EXPECTED_LABELS[slug]:
                check(label in actions, f"{slug}: renders CTA action `{label}`")
            check(
                len(actions) >= 2,
                f"{slug}: at least two action links (got {len(actions)})",
            )
            ...
            # Focus-visible ring on every interactive element. All four
            # variants ship real action links; each must show a 2px ring.
            ...
            links = page.query_selector_all(
                "#ds-root section a[href]"
            )
            check(
                len(links) >= 2,
                f"{slug}: at least two interactive elements (got {len(links)})",
            )
...
def generator_checks() -> None:
    _gen_script = ROOT / "scripts" / "tooling" / "generators" / "_gen_react_sections_cta.py"
    ...
    [sys.executable, str(_gen_script), "--check"],
        ...
        check(False, "generator drift check unavailable (not in checkout): _gen_react_sections_cta.py" % _gen_script.name)
