#!/usr/bin/env python3
"""Playwright QA for the React Sections Pricing previews.
...
Verifies behavior-critical guarantees (not cosmetics):
  ...
  - structure: exactly one h2 per section, section aria-labelledby points at
    it, prices use tabular-nums, plan lists use real ul semantics
...
BASE = "http://localhost:8765/React/Sections/Pricing/"
SLUGS = [
    "minimal",
    "dark-premium",
    "bento",
...
def static_checks() -> None:
    ...
    for slug in SLUGS:
        folder = ROOT / "library" / "React" / "Sections" / "Pricing" / slug
        ...
        check(meta.get("family") == "Pricing", f"{slug}: metadata family")
        check(meta.get("subcategory") == "Pricing", f"{slug}: metadata subcategory")
        ...
        check(meta.get("id") == f"pricing-{slug}", f"{slug}: metadata id")
...
def browser_checks() -> None:
    with sync_playwright() as pw:
        ...
        for slug in SLUGS:
            ...
            # Prices are numeric data — they must use tabular-nums (§5.3).
            tabular = page.evaluate(
                """(() => {
                    const s = document.querySelector('#ds-root section');
                    return [...s.querySelectorAll('span, p, dd')]
                        .filter(el => /\\$\\d/.test(el.textContent) && el.children.length === 0)
                        .every(el => el.className.includes('tabular-nums')
                            || el.closest('.tabular-nums'));
                })()"""
            )
            check(tabular, f"{slug}: price figures use tabular-nums")
