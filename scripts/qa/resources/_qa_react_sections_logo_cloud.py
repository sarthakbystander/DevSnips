#!/usr/bin/env python3
"""Playwright QA for the React Sections Logo-Cloud previews.
...
Verifies behavior-critical guarantees (not cosmetics):
  ...
  - structure: exactly one h2 per section, section aria-labelledby points at
    it, wordmarks are real `<li>` items inside a `<ul>`, the expected number
    of brands renders, every glyph svg is aria-hidden (the adjacent name
  ...
  - generator: `scripts/tooling/generators/_gen_react_sections_logo_cloud.py --check` reports no
...
Run from the repo root with a static server on :8765:
    ...
    python3 scripts/_qa_react_sections_logo_cloud.py
"""
import json
import re
import subprocess
...
BASE = "http://localhost:8765/React/Sections/Logo-Cloud/"
SLUGS = [
    "minimal",
    "dark-premium",
    "bento",
...
# Expected rendered brand count per variant (authored defaults).
BRAND_COUNTS = {
    "minimal": 8,
    "dark-premium": 6,
    "bento": 8,
...
def static_checks() -> None:
    ...
    family_dir = ROOT / "library" / "React" / "Sections" / "Logo-Cloud"
    ...
    for slug in SLUGS:
        folder = ROOT / "library" / "React" / "Sections" / "Logo-Cloud" / slug
        ...
        check(meta.get("family") == "Logo-Cloud", f"{slug}: metadata family")
        check(meta.get("subcategory") == "Logo-Cloud", f"{slug}: metadata subcategory")
        ...
        check(meta.get("id") == f"logo-cloud-{slug}", f"{slug}: metadata id")
...
def browser_checks() -> None:
    with sync_playwright() as pw:
        ...
        for slug in SLUGS:
            ...
            # Structure: one h2, aria-labelledby wiring, real list semantics.
            ...
            brand_items = page.evaluate(
                """[...document.querySelectorAll('#ds-root section ul li')]
                   .filter(li => li.querySelector('svg')).length"""
            )
            check(
                brand_items == BRAND_COUNTS[slug],
                f"{slug}: renders {BRAND_COUNTS[slug]} brand wordmark items (got {brand_items})",
            )
            glyphs = page.evaluate(
                "document.querySelectorAll('#ds-root section svg').length"
            )
            check(glyphs >= 1, f"{slug}: at least one glyph svg (got {glyphs})")
            bad_glyphs = page.evaluate(
                """[...document.querySelectorAll('#ds-root section svg')]
                   .filter(el => el.getAttribute('aria-hidden') !== 'true').length"""
            )
            check(bad_glyphs == 0, f"{slug}: every glyph svg is aria-hidden")
...
def generator_checks() -> None:
    _gen_script = ROOT / "scripts" / "tooling" / "generators" / "_gen_react_sections_logo_cloud.py"
    ...
    [sys.executable, str(_gen_script), "--check"],
        ...
        check(False, "generator drift check unavailable (not in checkout): _gen_react_sections_logo_cloud.py" % _gen_script.name)
