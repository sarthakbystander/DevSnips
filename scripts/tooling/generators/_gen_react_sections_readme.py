#!/usr/bin/env python3
"""Generate missing README.md files for React Sections.

Scans React/Sections/ for variant folders that are missing README.md and
...
def generate_readme_template(meta, variant_name):
    ...
    readme += "## Usage\n\n"
    readme += "```tsx\n"
    readme += "import Component from './code';\n\n"
    readme += "export default () => <Component />;\n"
    readme += "```\n"
...
def is_leaf(folder):
    """React section leaf: code.tsx + preview.html + metadata.json, no child metadata."""
...
def main(check_mode=False):
    ...
    for category in sorted(REACT_SECTIONS.iterdir()):
    ...
    if check_mode:
        if missing_count > 0:
            print(f"React Sections: {missing_count} missing README.md file(s)")
            return 1
        else:
            print("React Sections: all README.md files present")
            return 0
    else:
        print(f"Generated {generated_count} React Sections README.md files")
        return 0
