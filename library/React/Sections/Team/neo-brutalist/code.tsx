    ...
    >
      <div className="mx-auto max-w-[1280px] px-4 py-[clamp(4rem,3rem+4vw,6rem)] sm:px-6 lg:px-8">
        <header className="border-b-2 border-[var(--ds-color-border-strong)] pb-8">
          ...
          <h2
            id={headingId}
            className="mt-3 max-w-2xl text-[clamp(1.875rem,1.65rem+1vw,2.25rem)] font-bold leading-[1.15] tracking-[-0.02em]"
          >
            {title}
          </h2>
        ...
        <div className="mt-12 grid gap-8 lg:mt-16 lg:grid-cols-[1.35fr_1fr]">
          {featured ? (
            <article className={"min-h-[420px] p-6 sm:p-8 lg:p-10 " + PANEL}>
              <div className="flex items-start justify-between gap-8">
                <div>
                  ...
                  <h3 className="mt-5 text-[clamp(1.875rem,1.65rem+1vw,2.25rem)] font-bold leading-[1.15] tracking-[-0.02em]">
                    {featured.name}
                  </h3>
              ...
              <div className="mt-16 grid gap-6 border-t-2 border-[var(--ds-color-border-strong)] pt-6 sm:grid-cols-[1fr_auto] sm:items-end">
                <p className="max-w-[46ch] text-base leading-[1.5]">{featured.bio ?? "Making the hard calls and keeping the product useful."}</p>
                <div className="font-[var(--ds-font-mono)] text-xs font-semibold uppercase leading-[1.4] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">
            ...
            {members.slice(1).map((member, index) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  ...
                  <span className="font-[var(--ds-font-mono)] text-xs font-semibold uppercase leading-[1.4] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">
                    {member.location ?? "Remote"}
                  </span>
                </div>
                <h3 className="mt-7 text-lg font-bold leading-[1.25] tracking-[-0.01em]">{member.name}</h3>
                <p className="mt-1 font-[var(--ds-font-mono)] text-xs font-semibold uppercase leading-[1.4] tracking-[0.05em]">{member.role}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="mt-10 flex flex-col gap-5 border-t-2 border-[var(--ds-color-border-strong)] pt-7 sm:flex-row sm:items-end sm:justify-between">
          <p className="max-w-[58ch] font-[var(--ds-font-mono)] text-xs uppercase leading-[1.5] tracking-[0.04em] text-[var(--ds-color-muted-foreground)]">
