...
Menu items with meaningful leading icons in a consistent 16px aria-hidden slot — the text label still carries the accessible name, so meaning never depends on the icon alone.
...
Icons are the `icon` prop on `<DropdownMenuItem>` — a `ReactNode` rendered in a fixed 16px, aria-hidden, `currentColor` slot ahead of the label. Regular items tint the icon `--ds-color-muted-foreground`; destructive items inherit the destructive text color.
...
Icons are decorative by construction: they render inside an `aria-hidden` span and the visible text remains the accessible name. Never ship an icon-only menu item — if an item's meaning is not obvious from words, improve the words.
...
The icon slot is a fixed 16px (`[&_svg]:size-4`) with `shrink-0`, so labels stay aligned down the column even when some items have no icon. Pass any SVG that sizes itself to `currentColor`; no icon library is required.
