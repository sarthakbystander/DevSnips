...
Related actions grouped under readable section labels with role=group semantics and hairline separators — an account/workspace switcher pattern, not a card stack.
...
Sections are composition, not new primitives: `<DropdownMenuLabel>` renders the heading (uppercase, tracked, smaller — distinguishable without color) and `<DropdownMenuGroup>` wraps the items in `role="group"`. Wire them together with an `id` on the label and `aria-labelledby` on the group, and separate sections with `<DropdownMenuSeparator>`.
...
Groups announce their label through the `aria-labelledby` association, so screen-reader users hear the section context when moving between groups. Labels and separators are skipped by arrow-key navigation automatically because only menu items are focus stops.
...
Keep sections short (2–5 items) and few (2–3 per menu). If a menu grows past three labelled sections, the information architecture wants a settings page, not a bigger menu.
