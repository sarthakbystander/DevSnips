...
/**
 * DevSnips React Dropdown Menu — reference implementation.
 *
 * A compound action menu following the WAI-ARIA menu button pattern:
 * a real `<button>` trigger (`aria-haspopup="menu"` + `aria-expanded`),
 * a `role="menu"` panel of `role="menuitem"` buttons, real DOM focus
 * (arrow keys move actual focus between items), Escape / outside pointer
 * close with focus restored to the trigger, and a viewport-aware
 * placement flip (bottom/top × start/end) with no positioning dependency.
 *
 * Composition: `<DropdownMenu>` (root state) + `<DropdownMenuTrigger>` +
 * `<DropdownMenuContent>` + `<DropdownMenuItem>` / `<DropdownMenuLabel>` /
 * `<DropdownMenuGroup>` / `<DropdownMenuSeparator>`. Checkbox, radio, and
 * submenu primitives extend this same core in their own variants.
 */
