...
Confirmation of a completed operation: a green tint derived from the `color.success` token, the check glyph, and a polite `role="status"` — for "it worked" feedback after saves, payments, backups, and syncs.
...
`variant="success"` supplies the tint, the check glyph, and the polite role — confirm the outcome in the title, state the consequence in the description.

## Behavior

The success variant confirms that an operation finished. It announces politely (`role="status"`) — a confirmation should never interrupt the user's next task.
...
For transient success feedback after an async action (save, export), mount the alert when the operation resolves — see `alert-live` for the dynamic pattern.
