...
export interface TimezoneOption {
  value: string;
  label: string;
  /** e.g. "GMT-5", "GMT+1". Shown in the listbox + trigger in mono. */
  offset?: string;
  disabled?: boolean;
}
...
/**
 * Single-select listbox of timezones. The trigger shows the selected
 * timezone's label; each option row shows the offset (e.g. "GMT-5") in a mono
 * font followed by the timezone label. Full keyboard nav (ArrowUp/Down/Home/
 * End/Enter/Space/Escape/Tab), the ARIA listbox pattern (`aria-haspopup`/
 * `aria-expanded`/`aria-controls`/`aria-activedescendant`, `role="listbox"`,
 * `role="option" aria-selected`), and outside-click close. Controlled
 * (value/onChange) + uncontrolled (defaultValue). Visually consistent with the
 * base select language — offset rendered in `font-mono` muted foreground.
 */
export function TimezoneSelect({
  ...
  const triggerId = id ?? `timezone-select-${generatedId}`;
        ...
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selected && selected.offset ? (
              <span className="shrink-0 font-mono text-[12px] text-[var(--ds-color-muted-foreground)]">
                {selected.offset}
              </span>
            ) : null}
...
function TimezonePanel({
  ...
  triggerId: string;
  onChoose: (o: TimezoneOption) => void;
      ...
      {options.map((option, i) => (
        ...
        >
          <span className="flex min-w-0 items-center gap-2">
            {option.offset ? (
              <span className="w-16 shrink-0 font-mono text-[12px] text-[var(--ds-color-muted-foreground)]">
                {option.offset}
              </span>
            ) : null}
