...
function ActionIcon({ action }: { action: SelectAction }) {
...
/**
 * Custom accessible select whose dropdown panel has ACTION ROWS in a footer
 * area separated from the listbox by a `border-t` divider. The listbox keeps
 * the full WAI-ARIA combobox/listbox pattern (trigger button with
 * `aria-haspopup="listbox"`, `aria-expanded`, `aria-controls`,
 * `aria-activedescendant`; panel `role="listbox"` of `role="option"` rows
 * with `aria-selected`). The actions below it are separate focusable
 * `<button type="button">` items in the panel footer (reachable by Tab and
 * by ArrowDown past the last option). Activating an action calls its
 * `onSelect` and closes the panel. Keyboard: ArrowUp/Down/Home/End/Enter/
 * Space/Escape/Tab on the trigger; outside-click + Escape close. Controlled
...
export function SelectWithActions({
  ...
  function runAction(action: SelectAction) {
...
function ListboxPanel({
  ...
  return (
    ...
    >
      ...
      {actions.length > 0 ? (
        <div className="mt-1 border-t border-[var(--ds-color-border)] pt-1">
