...
Supporting description text between the label and the control — frames the field's purpose and is linked to the control with aria-describedby.
...
A description frames the field **before** the control (purpose, impact); a helper sits **below** it (format, constraints). Reach for `form-field-with-helper` for the latter.
...
The demo renders the same pattern over three different controls (input, select, textarea) — the description wiring is identical for each because `FormFieldControl` injects it.
