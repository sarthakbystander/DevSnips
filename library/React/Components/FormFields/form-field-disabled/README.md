...
`disabled` on `FormField` is forwarded to the wrapped control as the native attribute — including composite controls that forward props (the DevSnips contract).
...
The demo pairs a statically disabled input + select with a dynamic one: a “Lock security settings” checkbox toggles `disabled` on a field wrapping a composite switch control, proving the wiring reaches through composition. Disabled fields are not submitted with the form — use read-only controls when the value must be submitted.
