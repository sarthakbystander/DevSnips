...
The structural layer around a form control: label, description, control, helper, and validation message with automatic id / aria-describedby / aria-invalid wiring.
...
This reference shows the full vertical composition with four different controls — a text input, an email input, a `<select>`, and a `<textarea>` — proving the field wrapper is control-agnostic.
