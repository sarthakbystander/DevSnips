...
Guideline: when most fields in a form are required, mark the optional ones (`optional`); when most are optional, mark the required ones (`required` on `FormField`). The demo mixes both conventions so the indicators can be compared side by side.
