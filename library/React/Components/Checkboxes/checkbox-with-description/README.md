...
```tsx
<CheckboxWithDescription label="Two-factor authentication" description="Require a second factor at sign-in." defaultChecked />
```
...
Native checkbox with a bold label and a description block stacked beneath it. The description is linked with `aria-describedby`. The control is top-aligned so the box lines up with the label.
...
Use this when the option needs more than a one-line label to be understood — e.g. a setting whose effect needs a sentence of explanation.
