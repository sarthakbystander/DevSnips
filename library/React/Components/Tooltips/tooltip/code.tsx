...
/**
 * DevSnips React Tooltip — reference implementation.
 *
 * A compound, non-interactive tooltip following the WAI-ARIA tooltip
 * pattern: the trigger is a real focusable element (`aria-describedby`
 * pointing at the tooltip), the tooltip is `role="tooltip"` content that
 * opens on pointer hover (after `delayDuration`) AND on keyboard focus
 * (immediately), and dismisses on pointer leave, blur, or Escape.
 *
 * Composition: `<Tooltip>` (root state + placement config) +
 * `<TooltipTrigger>` (clones its single child element to attach the
 * trigger ref, event handlers, and `aria-describedby`) +
 * `<TooltipContent>` (the `role="tooltip"` bubble with a pointing arrow).
 * Placement is prop-driven (`side` × `align`), measured against the
 * viewport before paint, and flipped/shifted when the preferred placement
 * would overflow — no positioning dependency.
 *
 * A tooltip is supplementary: it never carries interactive content and is
