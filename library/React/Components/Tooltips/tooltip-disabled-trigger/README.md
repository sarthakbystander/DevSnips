...
```tsx
import Tooltip, { TooltipTrigger, TooltipContent } from "./tooltip-disabled-trigger";
...
The wrapper pattern needs no component support: the `<span tabIndex={0}>` is simply the trigger's child element, and the inner disabled control is made `pointer-events-none` so pointer events land on the wrapper. `TooltipTrigger` attaches everything to the wrapper. The `disabled` prop on the root is the other direction — it turns a tooltip off while leaving the control itself enabled.
