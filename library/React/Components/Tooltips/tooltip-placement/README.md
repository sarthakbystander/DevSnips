...
The full placement matrix through props — side × align on one shared core — plus the viewport correction: a trigger at the page edge flips its tooltip instead of overflowing.

## Usage

```tsx
import Tooltip, { TooltipTrigger, TooltipContent } from "./tooltip-placement";
...
All twelve combinations below are the same three primitives; only the `side` / `align` props change. The arrow follows the resolved placement — including after a flip.
...
Placement is purely visual: the accessibility tree only ever sees the trigger and its `aria-describedby` description, regardless of side, alignment, or a flip.
