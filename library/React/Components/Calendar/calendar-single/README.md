...
Explicit single-date selection, controlled by the parent: the selected date drives a milestone summary, re-selecting the same day is a no-op, and clearing is only possible through the explicit external action.
...
```tsx
const [date, setDate] = useState<Date | null>(new Date(2026, 7, 12));
...
Because selection is controlled, the summary text and the grid can never disagree: both render from the same state. The `aria-selected` gridcell, the filled day button, and the `role=status` summary all update together.
...
Demonstrates the controlled half of the single-selection API. Contrast with the reference variant (`calendar`), which is uncontrolled. The `Clear` action shows that emptying the selection is a deliberate parent decision — `selected={null}` is a valid controlled state.
