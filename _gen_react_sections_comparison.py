...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument('--check',action='store_true'); args=p.parse_args(); drift=[]
    for slug in VARIANTS:
        folder=BASE/slug; source=folder/'code.tsx'; preview=folder/'preview.html'
        if not source.exists(): print(f'ERROR: missing {source}',file=sys.stderr); return 1
        expected=render_preview(slug)
        if args.check:
            if not preview.exists() or preview.read_text()!=expected: drift.append(slug)
        else:
            preview.write_text(expected); print(f'wrote {preview.relative_to(ROOT)}')
    if drift: print('Drift detected in: '+', '.join(drift)); print('Run: python _gen_react_sections_comparison.py'); return 1
    if args.check: print('All Comparison previews are up to date.')
    return 0
