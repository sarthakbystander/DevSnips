...
Usage (works from any CWD, no hardcoded paths):
  python3 website/gen_site.py

Branding assets are read from this directory (website/):
  - style_extra.css  -> copied to library/assets/style.css  (light cadet gray theme)
  - logo.svg         -> original uploaded mark kept for reference; an
                        adaptive copy is written to library/assets/logo.svg
...
# CWD and any machine: script lives in <repo>/website/, site is emitted to <repo>/library/.
GEN_DIR = os.path.dirname(os.path.abspath(__file__))
ROOT = os.path.dirname(GEN_DIR)
SITE = os.path.join(ROOT, "library")
...
def ensure_local_src(it):
    ...
    (as _src/) so the deployed library/ is fully self-contained — no reaching
    outside the library root for previews/code, so it works no matter where
    library/ is deployed (with or without a /library/ prefix)."""
...
# ---- TECH HUB (e.g. library/Tailwind/index.html) ----
...
# ---- CATEGORY INDEX (e.g. library/Tailwind/Components/index.html) ----
...
def build_item(it, related_pool):
  ...
  <section class="mx-auto max-w-6xl px-4 py-8 sm:px-6">
    ...
    # Per-item machine-readable resource descriptor (self-describing for LLMs / tools)
    resource = {
        "id": f"{it['tech']}/{it['category']}/{it['family']}/{it['slug']}",
        "type": "component" if it["category"] == "Components" else "section",
        "technology": it["tech"],
        ...
        "url": f"{it['tech']}/{it['category']}/{it['family']}/{it['slug']}/index.html",
    }
    write(os.path.join(out_dir, "resource.json"), json.dumps(resource, indent=2, ensure_ascii=False) + "\n")
