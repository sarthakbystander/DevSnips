...
Open state lifted to the parent: a copy action forces its confirmation tooltip open for a moment, an external toggle pins a hint open, and every request flows through onOpenChange.

## Usage

```tsx
import Tooltip, { TooltipTrigger, TooltipContent } from "./tooltip-controlled";
...
First demo: the copy tooltip is fully parent-owned — hover/focus requests are logged through `onOpenChange` but ignored, and clicking Copy forces the tooltip open for 1.6 seconds as confirmation feedback. Second demo: an external Show/Hide toggle pins a hint open without touching the trigger. The log line proves every state change flows through one callback.
