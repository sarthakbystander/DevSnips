...
export interface SelectWithLeadingIconProps {
  label?: string;
  helperText?: string;
  error?: string;
  success?: string;
...
/**
 * Accessible custom select with a leading icon slot inside the trigger. The
 * `leadingIcon` ReactNode is rendered absolutely at `left-3`; the trigger gets
 * `pl-9` to make room. The icon is decorative — it carries `aria-hidden` so
 * it is excluded from the a11y tree. Implements the full WAI-ARIA combobox /
 * listbox pattern: trigger button with `aria-haspopup="listbox"`,
 * `aria-expanded`, `aria-controls`, `aria-activedescendant`; panel with
 * `role="listbox"` of `role="option"` rows carrying `aria-selected`. Keyboard:
 * ArrowUp/Down/Home/End/Enter/Space/Escape/Tab. Outside-click + Escape close.
 * Controlled (value/onChange) and uncontrolled (defaultValue) both supported.
 */
export function SelectWithLeadingIcon({
  label = "Select",
  helperText,
  error,
  success,
  ...
  const icon = leadingIcon !== undefined && leadingIcon !== null ? leadingIcon : <DefaultSearchIcon />;
  ...
  return (
    <div className="w-full">
      ...
      {message ? (
        <p id={messageId} className={cx("mb-2 text-xs", error ? "text-[var(--ds-color-destructive)]" : success ? "text-[var(--ds-color-success)]" : "text-[var(--ds-color-muted-foreground)]")}>
      ...
      <div className="relative">
        ...
        <button
          ...
          onKeyDown={onKeyDown}
          className={cx(
            "inline-flex w-full items-center justify-between gap-2 rounded-[var(--ds-radius-sm)] border bg-[var(--ds-color-input)] pl-9 pr-3 text-left text-[var(--ds-color-foreground)] transition-colors duration-150 ease-out hover:bg-[var(--ds-color-input-hover,var(--ds-color-input))] focus:bg-[var(--ds-color-input-focus,var(--ds-color-input))] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ds-color-focus-ring)] motion-reduce:transition-none",
            error ? "border-[var(--ds-color-destructive)]" : success ? "border-[var(--ds-color-success)]" : "border-[var(--ds-color-border)] focus:border-[var(--ds-color-border-strong)]",
            SIZES[size],
            className,
          )}
        >
          <span className="pointer-events-none absolute left-3 flex items-center text-[var(--ds-color-muted-foreground)] [&_svg]:size-4" aria-hidden="true">
            {icon}
          </span>
...
export default SelectWithLeadingIcon;
