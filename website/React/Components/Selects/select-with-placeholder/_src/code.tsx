...
export interface SelectWithPlaceholderProps {
  label?: string;
...
/**
 * Custom accessible select (WAI-ARIA combobox/listbox pattern) emphasizing
 * a prominent placeholder when nothing is selected. The placeholder
 * defaults to "Choose…" and is rendered in muted foreground with an inline
 * leading dot glyph so the empty/unselected state reads as intentional
 * rather than blank. The moment a value is chosen the placeholder is
 * replaced by the selected option label in the normal foreground color.
 * Full keyboard nav (ArrowUp/Down/Home/End/Enter/Space/Escape/Tab),
 * role=listbox of role=option rows with aria-selected, outside-click close,
 * and controlled (value/onChange) + uncontrolled (defaultValue) support.
 */
      ...
      <div className="relative">
        ...
        >
          {selected ? (
            ...
            <span className="flex flex-1 items-center gap-1.5 truncate text-[var(--ds-color-muted-foreground)]">
              <span className="inline-block size-1.5 rounded-full bg-[var(--ds-color-muted-foreground)]" aria-hidden="true" />
