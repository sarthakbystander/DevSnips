...
The `error` string is the whole validation contract: while it is non-empty the input carries `aria-invalid="true"` and the message renders with `role="alert"`; the helper text hides so exactly one message region owns the `aria-describedby` slot (description + error, in order).
...
The error message is referenced by the input's `aria-describedby` and announces with `role="alert"`; `aria-invalid` flips while the error is non-empty and clears on a successful pick. Contrast with `date-picker-with-label`, where the helper owns the message slot.
...
The pattern stays valid without the form demo: `error` can be driven by any validator. The destructive border tracks the error state (border + message + aria — never color alone).
