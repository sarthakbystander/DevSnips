...
Informational keyboard shortcuts aligned to the trailing edge of menu items — exposed via aria-keyshortcuts, never part of the accessible name, with stable right alignment.
...
Shortcuts are the `shortcut` prop on `<DropdownMenuItem>` — a string rendered in a trailing, `ml-auto`, `pl-6` column. The label keeps `min-w-0 flex-1 truncate`, so a long label truncates before it can push the shortcut column out of alignment.
...
The shortcut glyph is `aria-hidden` and mirrored on the item as `aria-keyshortcuts`, so assistive technology can announce the accelerator without it becoming part of the accessible name (activating the item still announces just the label).
...
Shortcuts use the platform's modifier notation in the demo (Ctrl+…); a real app would swap ⌘/Ctrl by platform. The same string is passed to `aria-keyshortcuts`.
