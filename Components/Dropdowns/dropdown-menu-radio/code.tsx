...
/**
 * DevSnips React Dropdown Menu — Radio Group.
 *
 * The shared menu core plus `<DropdownMenuRadioGroup>` +
 * `<DropdownMenuRadioItem>`: mutually exclusive options
 * (`role="menuitemradio"` + `aria-checked`) for settings like theme or
 * sort order. Controlled (`value` + `onValueChange`) and uncontrolled
 * (`defaultValue`) modes; exactly one option is checked at a time.
 */
...
/** Groups mutually exclusive `DropdownMenuRadioItem` options (`role="group"`). */
export function DropdownMenuRadioGroup({
  ...
  return (
    <DropdownMenuRadioGroupContext.Provider value={{ value: actualValue, select }}>
      <div role="group" {...rest}>
        {children}
      </div>
...
export interface DropdownMenuRadioItemProps
  extends ButtonHTMLAttributes<HTMLButtonElement> {
  /** The value this option represents inside the surrounding `DropdownMenuRadioGroup`. */
  value: string;
  /** Whether activating the item closes the menu (default false). */
  closeOnSelect?: boolean;
  /** Called when the item is activated, after the group value updates. Call `event.preventDefault()` to keep the menu open. */
  onSelect?: (event: ReactMouseEvent<HTMLButtonElement>) => void;
  children?: ReactNode;
}

/**
 * One option in a radio group (`role="menuitemradio"` + `aria-checked`).
 * Exactly one item in the group is checked; the dot indicator is aria-hidden
 * decoration tracking that state.
 */
export function DropdownMenuRadioItem({
  ...
  const menu = useDropdownMenu("DropdownMenuRadioItem");
  ...
  return (
    ...
    >
      <span
        aria-hidden="true"
        className={cx(
          "inline-flex size-4 shrink-0 items-center justify-center text-[var(--ds-color-primary)] transition-opacity duration-150 ease-out motion-reduce:transition-none [&_svg]:size-3",
