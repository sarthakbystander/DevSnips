...
/**
 * DevSnips React Calendar — reference implementation.
 *
 * A compound, accessible calendar following the WAI-ARIA date-picker grid
 * pattern: a `role="grid"` day matrix with `role="row"` / `role="gridcell"`
 * structure, real `<button>` day cells, roving tabindex keyboard navigation,
 * and locale-aware labels throughout.
 *
 * Composition: `<Calendar>` (root provider — selection mode, visible month,
 * constraints, locale) + `<CalendarHeader>` + `<CalendarPrevious>` /
 * `<CalendarNext>` (month / year / decade navigation) + `<CalendarHeading>`
 * (live month-year label; activates the month / year picker views) +
 * `<CalendarGrid>` (one month matrix; render several for `numberOfMonths`) +
 * `<CalendarFooter>` (summary / action region).
 *
 * Date model: calendar dates are LOCAL dates (no UTC conversion anywhere).
 * All arithmetic goes through `new Date(year, month, day)` constructor
 * normalization — never timestamp math (breaks across DST) and never string
 * comparison. Day identity is a numeric key (`year * 10000 + (month+1) * 100
 * + day`), strictly monotonic with calendar order.
 *
 * Selection modes: `single` (default — one date, clicking the selected date
 * does NOT deselect it), `multiple` (toggle dates, immutable array updates),
 * and `range` (`{ from, to }`; clicking an earlier date or a date that would
 * cross a disabled day restarts the range; clicking `from` completes a
 * same-day range). Every mode is controlled (`selected` + `onSelect`) or
