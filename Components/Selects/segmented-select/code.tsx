...
/**
 * Segmented single-select. A horizontal row of segments (buttons) where
 * exactly one is active at a time — radiogroup semantics: the wrapper carries
 * role="radiogroup" and each segment role="radio" aria-checked. Keyboard uses
 * roving tabindex: ArrowLeft/Right move between enabled segments and Tab
 * enters/exits the group (only the active segment is tabbable). The active
 * segment is bg-surface-active + font-medium + a border-strong edge; inactive
 * segments are transparent + muted. Controlled (value/onChange) + uncontrolled
 * (defaultValue). NOT a dropdown — no panel, no chevron.
 */
export function SegmentedSelect({
  ...
  function enabledIndex(start: number, step: number): number {
  ...
  function onKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    switch (event.key) {
      ...
      case "Home":
        event.preventDefault();
        for (let i = 0; i < options.length; i++) if (!options[i].disabled) { focusSegment(i); break; }
        break;
      case "End":
        event.preventDefault();
        for (let i = options.length - 1; i >= 0; i--) if (!options[i].disabled) { focusSegment(i); break; }
        break;
    }
  }
          ...
          return (
            <button
              ...
              className={cx(
                ...
                isActive
                  ? "border border-[var(--ds-color-border-strong)] bg-[var(--ds-color-surface-active)] font-medium text-[var(--ds-color-foreground)] shadow-[var(--ds-shadow-xs)]"
                  : "border border-transparent bg-transparent text-[var(--ds-color-muted-foreground)] hover:bg-[var(--ds-color-surface-hover)] hover:text-[var(--ds-color-foreground)]",
