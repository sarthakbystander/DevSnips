...
/**
 * Custom accessible select (WAI-ARIA combobox/listbox pattern) with an
 * error state. When `error` is present the trigger carries `aria-invalid`
 * and a destructive border `border-[var(--ds-color-destructive)]`, and the
 * error message is rendered below the field in destructive text and linked
 * via `aria-describedby` so assistive tech announces the validation
 * problem. The error is also prefixed with an inline alert glyph so the
 * state is not communicated by color alone. Full keyboard nav (ArrowUp/
...
export function SelectWithError({
  ...
  return (
    <div className="w-full">
      ...
      {error ? (
        <p id={messageId} className="mb-2 flex items-center gap-1.5 text-xs text-[var(--ds-color-destructive)]">
      ...
      <div className="relative">
        ...
        <button
          ...
          className={cx(
            ...
            error ? "border-[var(--ds-color-destructive)] focus:border-[var(--ds-color-destructive)]" : "border-[var(--ds-color-border)] focus:border-[var(--ds-color-border-strong)]",
            SIZES[size],
            className,
          )}
        >
