...
export interface SelectWithDescriptionProps {
  label?: string;
  options: DescribedOption[];
  value?: string;
  defaultValue?: string;
  onChange?: (value: string, option: DescribedOption) => void;
  size?: SelectSize;
  placeholder?: string;
  disabled?: boolean;
  id?: string;
...
/**
 * Custom single-select listbox where each option has a label + description.
 * Option rows render the label (medium) and a muted `text-xs` description on
 * the line below, kept compact. The selected option shows a check on the
 * right. Full keyboard nav: ArrowUp/Down/Home/End move, Enter/Space selects,
 * Escape closes. Outside-click + Escape close. ARIA listbox + options
 * aria-selected. Controlled + uncontrolled.
 */
export function SelectWithDescription({
  ...
  const triggerId = id ?? `select-description-${generatedId}`;
  ...
  function choose(option: DescribedOption) {
...
function DescriptionPanel({
  ...
  triggerId: string;
  onChoose: (o: DescribedOption) => void;
      ...
      {options.map((option, i) => {
        ...
        return (
          <button
            ...
            className={cx(
              "flex w-full items-start justify-between gap-2.5 rounded-[var(--ds-radius-sm)] px-2.5 py-1.5 text-left text-[13px] text-[var(--ds-color-foreground)] transition-colors motion-reduce:transition-none focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ds-color-focus-ring)]",
          ...
          >
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[var(--ds-color-foreground)]">
                {option.label}
              </span>
              {option.description ? (
                <span className="mt-0.5 block truncate text-xs text-[var(--ds-color-muted-foreground)]">
                  {option.description}
                </span>
              ) : null}
            </span>
            {isSelected ? (
              <CheckIcon className="mt-0.5 shrink-0 text-[var(--ds-color-primary)]" />
            ) : null}
          </button>
        );
      })}
