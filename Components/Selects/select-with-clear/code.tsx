...
export interface SelectWithClearProps {
  label?: string;
  helperText?: string;
...
/**
 * Custom accessible single-select with a CLEAR button in the trigger.
 * Whenever a value is selected, an inline clear affordance appears between
 * the label and the chevron — a real `<button type="button">` with
 * `aria-label="Clear selection"` that stops propagation (so clicking it
 * clears the selection WITHOUT opening the listbox), sets the value to "",
 * and fires `onChange("", null)`. Clicking the trigger body still toggles
 * the panel. Full WAI-ARIA combobox/listbox pattern: trigger button with
        ...
        >
          ...
          {selected ? (
            <button
              ...
              onClick={(e) => clear(e)}
              className="flex size-5 shrink-0 items-center justify-center rounded-full text-[var(--ds-color-muted-foreground)] transition-colors hover:bg-[var(--ds-color-surface-active)] hover:text-[var(--ds-color-foreground)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ds-color-focus-ring)] motion-reduce:transition-none"
            >
