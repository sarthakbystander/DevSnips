...
Settings, help, and sign-out-style actions in the footer: real links and real buttons with the family's full interaction states, separated from navigation by the footer border, and fully keyboard accessible in both surfaces.
...
Three footer action rows — two links (Settings, Help) and one real button action (Sign out) with genuine demo feedback — below the standard grouped navigation. No user block: see `sidebar-with-user` for that composition.
...
Footer rows behave identically in the column, the rail, and the drawer — including closing the drawer when a footer LINK is activated (actions that do not navigate keep the drawer open).
...
Footer actions keep their accessible names in the rail (sr-only labels + tooltips), and the sign-out feedback is a `role="status"` region so the result is announced without moving focus.
...
Footer actions behave like the rest of the system:
...
Actions are ordinary callbacks. The demo's sign-out flow is local state; wire the real thing to your auth layer.
...
Establishes the footer-action pattern: links vs buttons chosen honestly, status feedback for non-navigating actions, and border separation.
