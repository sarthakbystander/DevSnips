...
Same native checkbox as the reference, but `label` is required and always wrapped in a `<label htmlFor>`, so clicking anywhere on the text toggles the control. Renders a `*` when `required`.

## Accessibility

Native `<input type="checkbox">` + `<label htmlFor>` association. `aria-invalid` for errors, `aria-describedby` for external helper/error text, visible `focus-visible` ring.
...
Use this when you only need a label and no helper or description. Reach for `checkbox-with-helper` or `checkbox-with-description` when extra context is needed.
