...
Large pagination (44px controls, size=lg) for prominent content navigation such as featured articles and case studies — comfortable touch targets.
...
Identical primitives to the reference — the only change is the default `size` of `"lg"` (44px controls). Pass `size="md"` to opt back into the default density.
...
The 44px control height meets comfortable touch-target guidance without any extra work, and every state (idle, current, disabled, focus-visible) scales with the same tokens.
...
Large controls wrap earlier, which is the intent: `flex-wrap` moves overflowing controls to a new line at full size instead of shrinking them. For prominent destinations, compose fewer numbers — previous/next plus two or three pages reads better than a dense row.
...
Use for prominent, content-first navigation: featured articles, case studies, editorial archives, marketing pages. For dense admin surfaces, prefer the compact variant.
