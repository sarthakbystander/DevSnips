...
import type { ChangeEvent, HTMLAttributes, LiHTMLAttributes, ReactNode } from "react";

/**
/**
 * DevSnips React Pagination — pagination combined with a page-size selector.
 * Built on the reference compound core, plus `<PaginationPageSize>`: an
 * explicitly labeled native `<select>` rendered OUTSIDE the `<nav>` landmark,
 * because choosing a page size is a filter, not navigation.
 *
...
export interface PaginationPageSizeProps {
  /** Selected page size (controlled). Omit to run uncontrolled. */
  value?: number;
  /** Initial page size (uncontrolled); defaults to the first option. */
  defaultValue?: number;
  /** Called with the selected page size whenever it changes. */
...
/**
 * Page-size selector ("Rows per page"). A separate, explicitly labeled native
 * `<select>` — deliberately NOT part of the `<nav aria-label="Pagination">`
 * landmark, because choosing a page size is a filter, not navigation. Styled
 * with the same input tokens as the DevSnips Select family.
 */
export function PaginationPageSize({
  ...
  const selectId = id ?? `page-size-${generatedId}`;
  const isControlled = value !== undefined;
  const [internal, setInternal] = useState<number>(defaultValue ?? options[0]);
  ...
  return (
    <div className={cx("flex items-center gap-2", className)}>
      <label
        htmlFor={selectId}
        className="whitespace-nowrap text-sm text-[var(--ds-color-muted-foreground)]"
      >
        {label}
      </label>
      <div className="relative">
        <select
          ...
          className="h-9 appearance-none rounded-[var(--ds-radius-sm)] border border-[var(--ds-color-border)] bg-[var(--ds-color-input)] pl-3 pr-8 text-sm text-[var(--ds-color-foreground)] transition-colors duration-150 ease-out hover:bg-[var(--ds-color-input-hover,var(--ds-color-input))] focus:border-[var(--ds-color-border-strong)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--ds-color-focus-ring)] motion-reduce:transition-none"
        >
          {options.map((option) => (
        ...
        <span
          aria-hidden="true"
          className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-[var(--ds-color-muted-foreground)] [&_svg]:size-3.5"
        >
          <svg
            viewBox="0 0 24 24"
            fill="none"
