...
export interface SwitchWithErrorProps {
  label: ReactNode;
  error?: string;
...
/**
 * Switch with an associated validation message. Sets `aria-invalid="true"`
 * and links the error message with `aria-describedby`, so the failure is
 * communicated beyond color. The track border takes the destructive token.
...
export function SwitchWithError({
  ...
  return (
    <div className={cx("flex flex-col gap-1", className)}>
      ...
      {hasError ? (
        <p id={messageId} role="alert" className="pl-[34px] text-xs leading-4 text-[var(--ds-color-destructive)]">
          {error}
        </p>
      ) : helperText ? (
        <p id={messageId} className="pl-[34px] text-xs leading-4 text-[var(--ds-color-muted-foreground)]">
          {helperText}
        </p>
      ) : null}
    </div>
