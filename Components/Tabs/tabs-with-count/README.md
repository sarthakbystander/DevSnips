...
Pass a real number through `count` — the chip stays readable at row level with tabular figures and never shifts layout as digits change.
...
The count sits inside the trigger, so it joins the accessible name (e.g. "Comments 12"). Keep counts honest — no padded statistics.

## States

Same selected / idle / focus / disabled states as the reference tabs. The count chip is a bordered pill with `tabular-nums` figures, so it stays visible on the selected background.
...
Use only when the number answers a real question (open comments, tracked files). Non-numeric markers belong in tabs-with-badge.
