...
The tablist carries `overflow-x-auto` with the scrollbar suppressed (`scrollbar-width:none` + webkit pseudo-element). No scroll buttons are needed — Arrow keys bring the focused tab into view.
...
Keyboard navigation is unaffected by the scroll — focus happens first and the browser brings the focused tab into view. The suppressed scrollbar only affects the visual chrome.

## States

Same selected / idle / focus / disabled states as the reference tabs. The list scrolls horizontally instead of wrapping; labels are never truncated. Focusing a tab (mouse or keyboard) scrolls it into view natively.
...
Use when the section list legitimately outgrows the viewport (many categories). Prefer fewer tabs or the vertical treatment when you can restructure.
