 ...
 * once it exists. The CLI only creates it when absent and otherwise
 * leaves it byte-for-byte untouched.
 */

const fs = require('node:fs');
const path = require('node:path');
const { getDevSnipsDirectory } = require('./config.js');

const AGENTS_FILENAME = 'AGENTS.md';

/**
 * Get the absolute path to the devsnips/AGENTS.md file.
 *
 * @returns {string} - Absolute path to AGENTS.md
 */
function getAgentsPath() {
  return path.join(getDevSnipsDirectory(), AGENTS_FILENAME);
}
...
This project uses DevSnips for UI resources. DevSnips resources are installed
under this \`devsnips/\` directory.
...
- \`config.json\` is managed by the DevSnips CLI -- do not edit it manually.
