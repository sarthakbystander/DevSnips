...
Persistent helper text below the control — format and constraint hints, linked to the control with aria-describedby.
...
Helper text is persistent guidance, not feedback — it never appears or disappears as the value changes. For validation feedback use `form-field-with-error` / `form-field-with-success`.
...
Helper text answers “how is this used” — character formats, constraints, defaults. Because it is linked with `aria-describedby`, screen readers announce it after the label when the control receives focus.
