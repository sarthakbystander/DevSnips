...
Success state: a success-token message with a check icon in a role=status live region, linked to the control with aria-describedby — positive validation feedback without color-only signaling.
...
Success uses the same `FormFieldMessage` primitive as error — `tone="success"` swaps the icon, color token, and live-region role. It never sets `aria-invalid`.
...
The demo checks username availability as you type (3+ characters → available) and shows a static verified-endpoint example. `role="status"` is a polite live region, so the confirmation is announced without interrupting the user.
