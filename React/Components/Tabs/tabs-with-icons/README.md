...
Icons are optional ReactNode content passed through `icon` — nothing requires an icon library; pass any element, for example an inline svg.
...
Icons are wrapped in an `aria-hidden` span — they carry meaning only as a visual aid; the text label remains the accessible name. Never add decorative icons that duplicate the label.

## States

Same selected / idle / focus / disabled states as the reference tabs. The icon inherits the trigger's text color, so it follows selected and idle states automatically.
...
Use icons when each section has a clear glyph (Downloads, Members, Settings). Skip the icon otherwise — optical consistency matters more than filling every tab.
