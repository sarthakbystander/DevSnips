...
```tsx
<SwitchWithError label="Two-factor authentication" error="Two-factor authentication is required for admin accounts." />
```
...
Native switch with an optional error message. When `error` is set the input gets `aria-invalid="true"`, the track border + checked fill take the destructive token, and the message is rendered with `role="alert"` and `aria-describedby`. The failure is communicated by border, fill, and text — not color alone.
...
The error state never relies on color alone — the track, the fill, and the message all change. Swap `error` for `helperText` to return to a neutral helper.
