...
/**
 * DevSnips React Dropdown Menu — Checkboxes.
 *
 * The shared menu core plus `<DropdownMenuCheckboxItem>`: checkable menu
 * items (`role="menuitemcheckbox"` + `aria-checked`) for toggling view
 * options. Controlled (`checked` + `onCheckedChange`) and uncontrolled
 * (`defaultChecked`) modes; activation keeps the menu open by default so
 * several options can be toggled without reopening.
 */
...
export interface DropdownMenuCheckboxItemProps
  ...
  /** Called with the next checked state when the item is activated. */
  onCheckedChange?: (checked: boolean) => void;
  /** Whether activating the item closes the menu (default false, so several options can be toggled without reopening). */
  closeOnSelect?: boolean;
  /** Called when the item is activated, after the state toggles. Call `event.preventDefault()` to keep the menu open. */
  onSelect?: (event: ReactMouseEvent<HTMLButtonElement>) => void;
  /** Informational keyboard shortcut shown at the trailing edge. */
  shortcut?: string;
  children?: ReactNode;
}

/**
 * A checkable menu item (`role="menuitemcheckbox"` + `aria-checked`). The
 * native button keeps Space/Enter activation; the check indicator is
 * aria-hidden decoration whose visibility tracks the checked state, so it
 * works in controlled (`checked` + `onCheckedChange`) and uncontrolled
 * (`defaultChecked`) modes alike.
 */
