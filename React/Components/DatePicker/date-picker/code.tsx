...
/**
 * DevSnips React Date Picker — reference implementation.
 *
 * A compound, accessible date picker following the WAI-ARIA date-picker
 * dialog pattern: a real (textually read-only) input + a real trigger button
 * open a non-modal `role="dialog"` popover holding the calendar grid.
 *
 * Composition: `<DatePicker>` (root provider — value, open state, visible
 * month, constraints, locale, format) + `<DatePickerInput>` +
 * `<DatePickerTrigger>` + `<DatePickerContent>` (the popover panel) +
 * `<DatePickerHeader>` (month/year navigation) + `<DatePickerCalendar>`
 * (the day matrix / month / year pickers) + `<DatePickerFooter>` +
 * `<DatePickerPresets>` + `<DatePickerToday>` / `<DatePickerClear>` /
 * `<DatePickerApply>` + `<DatePickerTime>` (date-time selection).
 *
 * The calendar core (date model, grid, keyboard navigation, month/year
 * picker views, constraints) is derived from the DevSnips Calendar family —
 * same local-calendar-date semantics, numeric day keys, constructor
 * arithmetic, roving tabindex, and `Intl` formatting — self-contained here
 * per the DevSnips snippet architecture (no cross-family imports). There is
 * no date-library dependency.
 *
 * Selection modes: `single` (default — one date) and `range`
 * (`DateRange | null`, `{ from, to }` with hover preview while the range is
 * incomplete). Both work controlled (`value` + `onChange`) or uncontrolled
 * (`defaultValue`). With `requireApply`, calendar interaction stages a draft
 * and only `<DatePickerApply>` commits it (Escape / outside close discards);
 * otherwise selection commits immediately. `withTime` adds real hour/minute
 * controls and keeps the popover open until Apply/Done.
 */
