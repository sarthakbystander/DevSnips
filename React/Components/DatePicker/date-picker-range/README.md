...
Two-month date range selection with a typed `DateRange` value (`{ from, to }`), live hover preview while the range is incomplete, an incomplete-range summary, and range clear — never an ambiguous string.
...
During the incomplete step, pointer movement previews the would-be range with a distinct `surface-hover` band (committed middle uses `surface-active`) — the committed start stays filled. The two grids are labelled with their own month/year; there is still exactly one tabbable day cell across both months.

## Responsive Behavior

`numberOfMonths: 2` renders both months below `sm` stacked vertically (the shared `flex-col sm:flex-row` container), so the popover stays fully inside the viewport at 375px.
...
The canonical range composition: multi-month grid, hover preview, footer clear. Presets and disabled dates compose the same way (see those variants).
