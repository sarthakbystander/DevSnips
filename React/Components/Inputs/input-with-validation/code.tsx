...
export interface InputWithValidationProps extends Omit<InputHTMLAttributes<HTMLInputElement>, "size" | "prefix"> {
  label?: string;
  helperText?: string;
  error?: string;
  success?: string;
...
export function InputWithValidation({ label = "Billing email", helperText = "Use the address that receives invoices.", validate = (value) => value.includes("@") ? undefined : "Enter a valid email address.", size = "md", id, className, value, defaultValue = "", onChange, ...props }: InputWithValidationProps) {
  const generatedId = useId(); const inputId = id ?? `input-${generatedId}`; const messageId = `${inputId}-message`;
  const [internalValue, setInternalValue] = useState(String(defaultValue ?? "")); const currentValue = value === undefined ? internalValue : String(value);
  const error = currentValue ? validate(currentValue) : undefined;
  ...
  return <div className="w-full"><label htmlFor={inputId} className="mb-2 block text-[13px] font-medium leading-5 text-[var(--ds-color-foreground)]">{label}</label><input id={inputId} type="email" className={cx(inputClasses(size, error ? "error" : currentValue ? "success" : "default"), className)} value={currentValue} onChange={handleChange} aria-invalid={error ? true : undefined} aria-describedby={messageId} {...props} /><p id={messageId} className={cx("mt-2 text-xs", error ? "text-[var(--ds-color-destructive)]" : currentValue ? "text-[var(--ds-color-success)]" : "text-[var(--ds-color-muted-foreground)]")}>{error ? `Error: ${error}` : currentValue ? "Success: Email format looks correct." : helperText}</p></div>;
}
