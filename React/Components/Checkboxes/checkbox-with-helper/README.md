...
```tsx
<CheckboxWithHelper label="Product updates" helperText="Sent at most once a week." defaultChecked />
```
...
Native checkbox + a label and a helper paragraph. The helper is linked to the input with `aria-describedby` so assistive tech announces it after the label. The helper is indented to align with the label text.
...
Use this when a label alone is not enough and a short helper line gives useful context. For validation messaging use `checkbox-with-error`.
