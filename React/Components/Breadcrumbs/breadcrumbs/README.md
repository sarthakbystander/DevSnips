...
Accessible breadcrumb navigation as a compound component: semantic nav landmark, ordered list, real anchor links, and an aria-current current page.
...
This is the reference composition — every other variant in the family uses the same six primitives and extends the same class constants, states, and accessibility model.
...
Give the `<nav>` a more specific `label` (for example `label="Store breadcrumb"`) when more than one breadcrumb trail lives on a page. The current page is distinguished by weight and color together — never by color alone.
