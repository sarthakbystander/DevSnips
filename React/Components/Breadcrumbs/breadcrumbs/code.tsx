...
/**
 * DevSnips React Breadcrumbs — accessible breadcrumb navigation as a
 * compound component. Reference implementation for the family.
 *
 * `<Breadcrumbs>` renders `<nav aria-label>` and provides the shared
 * separator through context. `<BreadcrumbList>` renders the ordered list.
 * `<BreadcrumbItem>` renders each level; `<BreadcrumbLink>` renders a real
 * anchor for navigable levels; `<BreadcrumbCurrent>` marks the current page
 * (`aria-current="page"`, plain text — not a link); `<BreadcrumbSeparator>`
 * renders decorative, `aria-hidden` structure between levels.
 */
