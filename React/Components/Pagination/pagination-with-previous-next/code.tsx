...
/**
/**
 * DevSnips React Pagination — previous/next-only page navigation for flows
 * where numbered pages add no value (readers, wizards, detail views). Built
 * on the reference compound core.
 *
 ...
 *
 * `<PaginationStatus>` renders a plain-text "Page X of Y" readout in an
 * `aria-live="polite"` region so page changes are announced without moving
 * focus. Previous disables on the first page, Next on the last page.
 */
...
export interface PaginationStatusProps extends HTMLAttributes<HTMLSpanElement> {
  /** Formats the status text; defaults to `Page {page} of {totalPages}`. */
  format?: (page: number, totalPages: number) => string;
  className?: string;
}

/**
 * Plain-text "Page X of Y" readout for previous/next-only pagination. The
 * `aria-live="polite"` region announces page changes without moving focus.
 */
export function PaginationStatus({ format, className, ...rest }: PaginationStatusProps) {
  const context = usePagination("PaginationStatus");
  const text = format
    ? format(context.page, context.totalPages)
