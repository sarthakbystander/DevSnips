...
/**
 * DevSnips React Navbar — reference implementation.
 *
 * A compound site-navigation system built on a real `<nav>` landmark: a
 * brand region, left/center/right content sections of real `<a>` links,
 * action buttons, a mobile disclosure (toggle + collapsible region), and
 * an accessible navigation dropdown (disclosure pattern — links stay real
 * anchors, no `role="menu"` on navigation). The root owns the mobile-menu
 * state (controlled via `open` + `onOpenChange`, or uncontrolled via
 * `defaultOpen`) and the responsive breakpoint; every dropdown owns its
 * own open state with roving DOM focus, Escape / outside-pointer close,
 * focus restoration, and viewport-aware alignment.
 *
 * Composition: `<Navbar>` (root) + `<NavbarBrand>` + `<NavbarContent>` +
 * `<NavbarSection>` + `<NavbarItem>` + `<NavbarLink>` / `<NavbarAction>` /
 * `<NavbarDropdown>` + `<NavbarToggle>` + `<NavbarMobile>` +
 * `<NavbarMobileContent>`. Dropdown: `<NavbarDropdownTrigger>` +
