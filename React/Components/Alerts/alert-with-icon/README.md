...
The leading icon slot in detail: semantic variants render their status glyph automatically, the `icon` prop replaces it with any `ReactNode`, and `icon={null}` suppresses it — the icon is always decorative to assistive technology.
...
The icon is the leading grid column of the root flex row; the text column keeps `min-w-0` so content wraps beside it. `AlertIcon` renders the variant glyph by default and wraps any custom glyph you pass as children (16px, currentColor, `aria-hidden`).

## Behavior

Three icon treatments, all through one prop:
...
The icon slot is 16px and colored by the variant's semantic token (muted for `default`). Pass your own SVG as `icon` — the slot sizes it with `[&_svg]`-free, currentColor-friendly geometry.
