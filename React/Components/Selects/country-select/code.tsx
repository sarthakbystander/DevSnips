...
export interface CountryOption {
  value: string;
  label: string;
  /** 2-letter ISO 3166-1 alpha-2 code, e.g. "US", "JP". */
  code: string;
  disabled?: boolean;
}
...
/**
 * Single-select listbox of countries. Each option is represented by a small
 * square badge showing the 2-letter ISO code (no emoji flags — a restrained
 * text badge keeps parity across platforms and themes) followed by the country
 * label. The trigger shows the selected country's code badge + label. Full
 * keyboard nav (ArrowUp/Down/Home/End/Enter/Space/Escape/Tab), ARIA listbox
 * pattern (`aria-haspopup`/`aria-expanded`/`aria-controls`/
 * `aria-activedescendant`, `role="listbox"`, `role="option" aria-selected`),
 * and outside-click close. Controlled (value/onChange) + uncontrolled
 * (defaultValue).
 */
export function CountrySelect({
  ...
  const triggerId = id ?? `country-select-${generatedId}`;
      ...
      <div className="relative">
        ...
        >
          <span className="flex min-w-0 flex-1 items-center gap-2">
            {selected ? <CodeBadge code={selected.code} /> : null}
...
function CodeBadge({ code, className }: { code: string; className?: string }) {
  return (
    <span
      aria-hidden="true"
      className={cx(
        "inline-flex h-4 min-w-[24px] shrink-0 items-center justify-center rounded-[2px] border border-[var(--ds-color-border-strong)] bg-[var(--ds-color-surface-subtle)] px-1 font-mono text-[10px] font-medium uppercase leading-none text-[var(--ds-color-foreground)]",
        className,
      )}
    >
      {code}
