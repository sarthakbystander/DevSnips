...
export function SelectWithLoading({
  ...
  return <div className="w-full">
      ...
      {message ? <p id={messageId} className={cx("mb-2 text-xs", error ? "text-[var(--ds-color-destructive)]" : "text-[var(--ds-color-muted-foreground)]")}>
  ...
  >
          ...
          {loading ? <Spinner className="shrink-0 size-4 animate-spin text-[var(--ds-color-muted-foreground)] motion-reduce:animate-none" /> : <ChevronDown className={cx("shrink-0 text-[var(--ds-color-muted-foreground)] transition-transform duration-150 motion-reduce:transition-none", open && "rotate-180")} />}
        </button>
        {open && !loading ? <ListboxPanel
    ref={listboxRef}
    id={listboxId}
