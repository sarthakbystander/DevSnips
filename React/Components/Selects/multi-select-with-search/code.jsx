...
function CheckIcon({ className }) {
  return <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
export function MultiSelectWithSearch({
  ...
  const summary = selectedValues.length === 0 ? placeholder : selectedValues.length <= 2 ? selectedOptions.map((o) => o.label).join(", ") : `${selectedValues.length} selected`;
  useEffect(() => {
    if (!open) return;
...
function MultiSearchPanel({
  ...
  >
    ...
    >
      ...
      isSelected ? "border-[var(--ds-color-primary)] bg-[var(--ds-color-primary)] text-[var(--ds-color-primary-foreground)]" : "border-[var(--ds-color-border-strong)] bg-[var(--ds-color-input)]"
    )}>
