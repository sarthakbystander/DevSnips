...
/**
 * Custom accessible select (WAI-ARIA combobox/listbox pattern) that ALWAYS
 * renders an associated <label htmlFor> paired with the trigger. The
 * showcase emphasis is the label + field pairing: the label is never
 * omitted, defaults to "Field", and is wired to the trigger id so clicking
 * the label focuses the control. Full keyboard nav (ArrowUp/Down/Home/End/
 * Enter/Space/Escape/Tab), role=listbox of role=option rows with
 * aria-selected, outside-click close, and controlled (value/onChange) +
