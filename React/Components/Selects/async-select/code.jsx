...
export function AsyncSelect({
  ...
  const [options, setOptions] = useState(() => isDefaultArray ? defaultOptions : []);
  ...
  function runLoad(query) {
    const token = ++reqRef.current;
    setIsLoading(true);
    setError(null);
    Promise.resolve().then(() => loadOptions(query)).then((res) => {
...
function ListboxPanel({
  ...
  >
      {error ? <div id={statusId} role="alert" className="flex items-center gap-2 px-2.5 py-2 text-[13px] text-[var(--ds-color-destructive)]">
          <AlertGlyph className="size-4 shrink-0" />
          <span className="truncate">{error}</span>
        </div> : isLoading ? <div id={statusId} className="flex items-center gap-2 px-2.5 py-2 text-[13px] text-[var(--ds-color-muted-foreground)]">
          <Spinner className="size-4 shrink-0 animate-spin motion-reduce:animate-none" />
          <span>{loadingPlaceholder}</span>
        </div> : options.length === 0 ? <div id={statusId} className="px-2.5 py-2 text-[13px] text-[var(--ds-color-muted-foreground)]">
          {emptyMessage}
        </div> : options.map((option, i) => <button
    key={option.value}
    id={`${triggerId}-opt-${i}`}
    type="button"
    data-i={i}
