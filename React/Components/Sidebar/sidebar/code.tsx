...
/**
 * DevSnips React Sidebar — reference implementation.
 *
 * A compound application-navigation system: a persistent desktop sidebar
 * (expanded 256px ↔ collapsed 64px icon rail, both real `<aside>` landmarks)
 * plus a mobile modal navigation drawer (overlay, focus trap, scroll lock,
 * Escape / outside-pointer close, focus restoration) rendered from the SAME
 * composition of children. One composition serves both — the desktop area
 * is hidden below the root breakpoint and the drawer is only reachable
 * below it, so the active landmark is always unique.
 *
 * Composition: `<SidebarProvider>` (state root) + `<Sidebar>` (landmarks) +
 * `<SidebarHeader>` / `<SidebarContent>` / `<SidebarFooter>` regions +
 * `<SidebarGroup>` / `<SidebarGroupLabel>` sections + `<SidebarMenu>` /
 * `<SidebarMenuItem>` / `<SidebarMenuButton>` navigation rows +
 * `<SidebarMenuCollapsible>` (expandable parents) + `<SidebarMenuSub>` /
 * `<SidebarMenuSubItem>` / `<SidebarMenuSubButton>` (nested levels) +
 * `<SidebarTrigger>` / `<SidebarRail>` (mode controls) + `<SidebarSearch>`
 * (navigation filter) + `<SidebarNav>` (typed data-driven renderer) +
 * `useSidebar()` (context escape hatch).
 */
