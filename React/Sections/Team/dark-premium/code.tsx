  ...
  return (
    <section data-theme="dark" aria-labelledby={headingId} className="bg-[var(--ds-color-background)] font-[var(--ds-font-sans)] text-[var(--ds-color-foreground)]">
      <div className="mx-auto max-w-[1280px] px-4 py-[clamp(4rem,3rem+4vw,6rem)] sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[5fr_7fr] lg:items-end">
          <header className="max-w-xl">
            <p className="font-[var(--ds-font-mono)] text-xs uppercase leading-[1.3] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">{eyebrow}</p>
            ...
            <p className="mt-4 text-[clamp(1rem,0.95rem+0.25vw,1.125rem)] leading-[1.5] text-[var(--ds-color-muted-foreground)]">{description}</p>
          </header>
          <div className="grid gap-px border border-[var(--ds-color-border)] bg-[var(--ds-color-border)] sm:grid-cols-2">
            {featured ? (
              <article className="group flex min-h-[360px] flex-col bg-[var(--ds-color-surface)] p-6 sm:col-span-2 sm:p-8">
                <div className="flex items-start justify-between gap-6 border-b border-[var(--ds-color-border)] pb-6">
                  <div>
                    <p className="font-[var(--ds-font-mono)] text-xs uppercase leading-[1.4] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">01 / Featured</p>
                ...
                <div className="mt-auto grid gap-6 pt-8 sm:grid-cols-[1fr_auto] sm:items-end">
                  <p className="max-w-[48ch] text-sm leading-[1.5] text-[var(--ds-color-muted-foreground)]">{featured.bio ?? "Bringing product, engineering, and craft together."}</p>
            ...
            {supporting.map((member, index) => (
              <article key={member.name} className="group flex min-h-[220px] flex-col bg-[var(--ds-color-surface)] p-6 transition-colors duration-150 ease-out hover:bg-[var(--ds-color-surface-hover)] motion-reduce:transition-none">
                <div className="flex items-start justify-between gap-4">
                  <p className="font-[var(--ds-font-mono)] text-xs uppercase leading-[1.4] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">{String(index + 2).padStart(2, "0")} / Team</p>
                ...
                <div className="mt-auto pt-8">
                  ...
                  {member.location ? <p className="mt-3 font-[var(--ds-font-mono)] text-xs uppercase leading-[1.4] tracking-[0.05em] text-[var(--ds-color-muted-foreground)]">{member.location}</p> : null}
