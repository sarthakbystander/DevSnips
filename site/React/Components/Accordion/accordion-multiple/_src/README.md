...
Nothing about the items changes in multiple mode — only the root's state shape. The controlled demo below shows the parent reading the open array to drive a live count and Expand all / Collapse all actions.

## Behavior

`type="multiple"` lets any number of items stay open: toggling an item adds or removes its value from the open array without touching the others. There is no `collapsible` concept — items always toggle freely.
...
- The controlled `value` must be an array in multiple mode; the discriminated union enforces it at compile time.
