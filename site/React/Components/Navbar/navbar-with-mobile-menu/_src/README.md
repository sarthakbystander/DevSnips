...
The responsive collapse pattern in depth: below the breakpoint the desktop navigation becomes a toggle-disclosed mobile region with aria-expanded / aria-controls wiring, Escape and outside-pointer close, and predictable focus — shown in both uncontrolled and controlled state modes.
...
The mobile region is a first-class part of the composition: `NavbarToggle` + `NavbarMobile` + `NavbarMobileContent`. The toggle references the actual region through `aria-controls`, and links inside `NavbarMobileContent` automatically render stacked and full-width.

## Navigation Behavior

Two navbars demonstrate the state model end to end (resize the preview below 768px to use them):
...
The two demo navbars carry distinct landmark labels (`Main` / `Controlled demo`) — required whenever more than one `<nav>` is on the page. The toggle's accessible name reflects state ("Open/Close navigation menu") and its icon is aria-hidden.
...
This variant exists to be resized: at 1280px the desktop row is fully visible and the toggle is hidden; at 768px the `md` breakpoint flips (desktop content hides, toggle appears); at 375px the panel is the only navigation. Both state modes behave identically at every width.
...
The controlled demo is the canonical example: `const [open, setOpen] = useState(false); <Navbar open={open} onOpenChange={setOpen}>`. Every close path — toggle, Escape, outside pointer, link activation — reports through `onOpenChange`.
...
Both demos render on one page, so each `Navbar` gets a distinct `label`. The controlled demo's readout text is showcase state, not part of the component.
