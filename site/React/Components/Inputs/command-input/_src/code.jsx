...
export function CommandInput({ label = "Command", commands = ["Open settings", "Invite member", "Create project"], size = "md", id, className, value, defaultValue = "", onChange, ...props }) {
  ...
  function handleChange(event) { if (value === undefined) setInternalValue(event.target.value); onChange?.(event); setOpen(true); }
  function choose(command) { if (value === undefined) setInternalValue(command); setOpen(false); }
