...
/**
 * Accessible custom select demonstrating the DISABLED state. When `disabled`
 * (default true) the trigger is non-interactive: `disabled`, `aria-disabled`,
 * a muted background `bg-[var(--ds-color-muted)]`, `opacity-60`, and
 * `pointer-events-none`. ARIA stays correct even when disabled — the listbox
 * pattern is fully implemented (combobox trigger, role=listbox, role=option,
 * aria-selected, full keyboard nav, outside-click close) and activates the
 * moment `disabled` becomes false. Controlled (value/onChange) and
  ...
  return (
    <div className="w-full">
      ...
      {message ? (
        <p
          id={messageId}
          className={cx("mb-2 text-xs", error ? "text-[var(--ds-color-destructive)]" : "text-[var(--ds-color-muted-foreground)]")}
        >
      ...
      <div className="relative">
        ...
        <button
          ...
          onClick={() => (disabled ? undefined : open ? setOpen(false) : openListbox())}
          onKeyDown={onKeyDown}
          className={cx(
            ...
            disabled
              ? "pointer-events-none border-[var(--ds-color-border)] bg-[var(--ds-color-muted)] text-[var(--ds-color-muted-foreground)] opacity-60"
              : cx(error ? "border-[var(--ds-color-destructive)]" : "border-[var(--ds-color-border)] focus:border-[var(--ds-color-border-strong)]"),
            SIZES[size],
            className,
          )}
        >
          ...
          <ChevronDown
            className={cx(
              "shrink-0 text-[var(--ds-color-muted-foreground)] transition-transform duration-150 motion-reduce:transition-none",
              open && "rotate-180",
            )}
          />
        </button>
