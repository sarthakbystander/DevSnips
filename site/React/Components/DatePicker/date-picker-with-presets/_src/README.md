...
A range date picker with a configurable preset panel (Today, Yesterday, Last 7 days, Last 30 days, This month): choosing a preset sets the actual typed range value and tracks the active preset with `aria-current`.
...
Presets pass through the same selection guard as manual picks (a preset whose days fall entirely inside `minDate`–`maxDate` still applies; constraints normally remain unset in preset demos).
...
Same as the base table. Preset buttons are ordinary tab stops; activating one behaves exactly like a manual completed pick (close + focus restore).
...
The preset panel is a real `role="group"` of buttons with an accessible group label — not a detachable decorative list. `aria-current="date"` marks the option matching the current value.

## Responsive Behavior

The preset column wraps horizontally below `sm` (shared `flex-col sm:flex-row` layout), so the popover never overflows at 375px.
...
Presets are a plain prop-driven list — a product registers exactly the entries it needs (they are not hard-coded inside the component). Ordering is stable; the active marker follows the value.
