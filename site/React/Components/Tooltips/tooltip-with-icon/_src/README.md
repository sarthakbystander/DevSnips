...
The toolbar row is the everyday use of this variant: five icon-only actions, each a 36px control with `aria-label` + matching tooltip. Keyboard users Tab through the row and get the same names as pointer users, with no hover delay.
